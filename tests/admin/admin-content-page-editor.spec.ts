...
test.describe("Admin Page Editor", () => {
  ...
  test("can publish and unpublish a page", async ({ page, request }) => {
    ...
    const createRes = await request.post("/api/admin/content/pages", {
      data: {
        title: "E2E Test Publish",
        slug: "e2e-test-publish",
      },
    });
    expect(createRes.ok()).toBeTruthy();
    const { page: testPage } = await createRes.json();
  ...
  test("warns about unsaved changes", async ({ page, request }) => {
    ...
    const createRes = await request.post("/api/admin/content/pages", {
      data: {
        title: "E2E Test Unsaved",
        slug: "e2e-test-unsaved",
      },
    });
    expect(createRes.ok()).toBeTruthy();
    const { page: testPage } = await createRes.json();
  ...
  test("rich text editor has formatting toolbar", async ({ page, request }) => {
    // Create a page with a text block
    const createRes = await request.post("/api/admin/content/pages", {
      data: {
        title: "E2E Test Rich Text",
        slug: "e2e-test-rich-text",
        content: {
          schemaVersion: 1,
          blocks: [
            { id: "text-1", type: "text", order: 0, data: { content: "<p>Test content</p>" } },
          ],
        },
      },
    });
    ...
    await page.locator('[data-test-id="block-item-text-1"]').click();

    // Should show text block editor with rich text toolbar
    await expect(page.locator('[data-test-id="text-block-editor"]')).toBeVisible();
    await expect(page.locator('[data-test-id="rich-text-editor"]')).toBeVisible();
  ...
  test("page list links to editor", async ({ page, request }) => {
    // Create a test page
    const createRes = await request.post("/api/admin/content/pages", {
      data: {
        title: "E2E Test Link",
    ...
    // Find and click on the page
    await page.locator(`a[href="/admin/content/pages/${testPage.id}"]`).click();

    // Should be on editor page
    await expect(page).toHaveURL(`/admin/content/pages/${testPage.id}`);
    await expect(page.locator('[data-test-id="page-editor-root"]')).toBeVisible();
  });
});
